...
export default function UploadImg({ url, onUpload }) {
  ...
  // async function uploadImgs(event) {
  //   try {
  //     setUploading(true);
  ...
  //     console.log("EVENT-", event.target.files.map((file) => file.name));
  //   }
  // }
  ...
  const onChange = (e) => {
    for (const file of e.target.files) {
      ...
      reader.onload = () => {
        setHeadshotSrc((imgs) => [...imgs, reader.result]);
      };
      reader.onerror = () => {
        console.log(reader.error);
      };
      onUpload(headshotSrc);
    }
  };
